...
public class AdminAddProductDto {
    ...
    public ProductEntity toEntity(ProductCategoryEntity component) { // 업로드할 사진이 없을 때 저장하는 함수
    ...
    public ProductEntity toEntity(List<String> imagePaths, ProductCategoryEntity component) { // 업로드할 사진이 있을 때 저장하는 함수
        if (this.quantity > 0) {stockStatus = ProductStockStatus.IN_STOCK;}
        else {stockStatus = ProductStockStatus.SOLD_OUT;}
        return ProductEntity.builder()
            .id(this.id)
